            ...
            {!content ? (
                ...
                <div className="space-y-6">
                    ...
                    {dramas.slice(0, 3).length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            {dramas.slice(0, 3).map((drama, index) => (
                                <Link key={drama.id || index} href={`/watch/${drama.id}`}>
                                    <Card className="group relative overflow-hidden border-0 bg-card hover-lift cursor-pointer">
                                        <div className="relative aspect-video bg-muted">
                                            <img
                                                src={drama.thumbnail || drama.poster || '/images/placeholder.jpg'}
                                                alt={drama.title}
                    ...
                    {dramas.slice(3).length > 0 && (
                        <div>
                            <h2 className="text-xl font-bold mb-4">Top Ranked</h2>
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
                                {dramas.slice(3).map((drama, index) => (
                                    <div key={drama.id || index} className="relative">
                                        {/* Rank Number */}
                                        <div className="absolute -top-2 -left-2 z-10 glass rounded-full w-8 h-8 flex items-center justify-center font-bold text-sm">
                                            #{index + 4}
                                        </div>
